              ...
              if (alert.type === 1) {
                return (
                    <Alert key={index} style={{textAlign: "left"}} color="danger">
                      <div onClick={() => closeAlert(alert.id)} className="close" data-dismiss="alert" aria-label="close" title="close">×</div>
                      <strong>[Nhắc nợ]</strong> Bạn nợ tài khoản {alert.account_id} ({alert.name}) số tiền {formatMoney(alert.money)} VNĐ.<br/>
                      Nhấn vào đây để{" "}
                      <Link to={`/transfer?account=${alert.account_id}&name=${alert.name}&money=${alert.money}&note=${alert.message}&debt=${alert.debt_id}`}>
                        Thanh toán{" "}
                ...
                return (
                    <Alert key={index} style={{textAlign: "left"}} color="success">
                      <div onClick={() => closeAlert(alert.id)} className="close" data-dismiss="alert" aria-label="close" title="close">×</div>
                      <strong>[Con nợ đã thanh toán]</strong> Con nợ có tài khoản {alert.account_id} ({alert.name}) đã thanh toán nợ với nội dung
                      :<br/>
                      {alert.message}<br/>
                      Xem <Link to="/debt">danh sách nhắc nợ</Link>
                    </Alert>
