  ...
  minus: (entity) => new Promise((resolve, reject) => {
    pool.getConnection((err, connection) => {
      ...
      connection.beginTransaction( (err) => {
        if (err) throw err
        connection.query('SELECT surplus, b.id FROM banking_account b JOIN user_info u ON b.id = u.id WHERE u.account_num=?', entity.to_account,
        (error, results, fields) => {
          ...
          if (surplus < entity.amount)
            ...
            connection.query('UPDATE banking_account SET ? WHERE ?', [{surplus: acc}, {owner_id: id}], (error, results, fields) => {
              if (error) 
                return connection.rollback(function () {
                  throw error
                })
